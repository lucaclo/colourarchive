...
import { analysisOutliers } from './group';
...
export async function runMatch(input: RunMatchInput, onStage: OnStage = () => {}): Promise<MatchRecord> {
  ...
  // see mergeReferences's own doc comment for why that has to hold exactly.
  const reference = mergeReferences(references.map((r) => ({ analysis: r.analysis, weight: r.weight })));
  // A separate question from the blend itself — see analysisOutliers's own
  // doc comment. Only worth asking below three references, which is a
  // refusal to guess rather than a clean bill.
